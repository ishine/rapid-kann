...
void kad_vec_mul_sum(int n, float *a, const float *b, const float *c) { /* dot product */
    int i;
    for (i = 0; i < n; ++i)
        a[i] += b[i] * c[i];
}
...
__global__ void cuda_sgemm_naive(const float *A, const int lda, const float *B, const int ldb, float *C, const int ldc, const int M, const int N, const int K) {
    ...
    if (row >= M || col >= N) return; /* N = c_width, M = c_height  */

    float acc = 0.0f;
    for (int k = 0; k < K; k++) { /* K = common_dim */
...
__global__ void cuda_sgemm_coalesce(const float *A, const int lda, const float *B, const int ldb, float *C, const int ldc, const int M, const int N, const int K) {
    ...
    for (a_ul = A_UL_begin, b_ul = B_UL_begin;
         a_ul < A_UL_end;
         a_ul += A_step, b_ul += B_step) {
        /* load A and B sub-blocks to shared memory */
        if (a_ul + threadIdx.x < A_UL_end && load_y_in_A) {
            As[threadIdx.y][threadIdx.x] = A[a_ul + lda * threadIdx.y + threadIdx.x];
        } else {
            As[threadIdx.y][threadIdx.x] = 0;
        ...
        if (load_x_in_B && b_ul + threadIdx.y * ldb < B_UL_end) {
            Bs[threadIdx.y][threadIdx.x] = B[b_ul + ldb * threadIdx.y + threadIdx.x];
        } else {
...
void kad_sgemm_simple(int trans_A, int trans_B, int M, int N, int K, const float *A, const float *B, float *C) /* simplified BLAS sgemm */
{
    ...
    int i, j, k;
    if (!trans_A && trans_B) /* B transposed */
    {
        for (i = 0; i < M; i += x)
            for (j = 0; j < N; j += x) {
    ...
    } else if (!trans_A && !trans_B) /* neither transposed */
    {
        for (i = 0; i < M; ++i)
            for (k = 0; k < K; ++k)
                kad_saxpy_inlined(N, A[i * K + k], &B[k * N], &C[i * N]); /* uses AVX/SSE */
    } else if (trans_A && !trans_B)                                       /* A transposed */
    {
        for (k = 0; k < K; ++k)
            for (i = 0; i < M; ++i)
...
static void conv2d_move_1to3(int d[4], const float *x, float *y) /* convert the NCHW shape to the NHWC shape - switch dimensions
mini-batch size, channels, height, width */
{
    int i, j, k, l;
    for (i = 0; i < d[0]; ++i)
        for (j = 0; j < d[1]; ++j)
...
#define conv2d_loop2(_x, _w, _y, _code)                                                                                                                \
    ...
    if (action == KAD_FORWARD || action == KAD_BACKWARD) { /* allocate working space */
        // printf("Dimensions of convolution are %d x %d = %d\n", w->d[3], w->d[1], w->d[3] * w->d[1]);
        if (w->d[3] * w->d[1] < 16) /* if conv kernel size < 16 use first algorithm */
        {
    ...
    if (action == KAD_SYNC_DIM) {
        ...
        memset(p->x, 0, kad_len(p) * sizeof(float));
        if (!algo_switch) /* both algorithms produce same results */
